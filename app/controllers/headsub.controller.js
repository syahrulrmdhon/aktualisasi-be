...
exports.update = (req, res) => {
  ...
  const imgdataHS = req.body.signature_headsub;
  let pathHS;
  const isBase64 = (str) => {
    return str.includes("base64");
  };

  if (imgdataHS && isBase64(JSON.stringify(imgdataHS))) {
    // path =
    //   __basedir +
    //   "/resources/uploads/" +
    //   Date.now() +
    ...
    return imagekit
      .upload({
        file: imgdataHS.replace(/^data:([A-Za-z-+/]+);base64,/, ""), //required
        fileName:
